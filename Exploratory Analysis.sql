...
-- The gender 'Other' makes more than both Male and Female at 125k.ABORT
...
--      2. Does higher education always result in higher salaries, or is there a threshold where the trend changes?

-- For this one, I think it would be best to create small bins for years of experience to condense the data.ABORT
...
-- From this we can see that 22 - 24 years of experience has the highest average salary, followed by 31+.
-- For question 2 -The higher years of experience : higher salary trend tends to plateau around 15-17 years of experience.
-- The trend is very strong from 0 to 15 years, then flattens off after that.ABORT
...
-- Now lets look at the bottom 10 jobs.ABORT
SELECT ROUND(AVG(salary), 2) as avg_sal, job_title
FROM salary_data
GROUP BY job_title
HAVING COUNT(job_title) >= 5
...
-- Lets look at the top 5 male dominated jobs.
...
-- Women tend to dominate the people side of businesses including HR, Social Media, Marketing, and Customer Service.ABORT
